...
const UserSettings = ({ isFullscreen, setIsFullscreen }: UserSettingsProps) => {
  ...
  const validate = (vals: PasswordValues): Record<string, string> => {
  ...
  const [newPomodoroTime, setNewPomodoroTime] = useState<number | string>(
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      setShowDeleteModal(false);
      addToast('error', (err as Error).message || 'Failed to delete account');
    }
  };
    ...
    <div className={styles.settings}>
      ...
      <section className={styles.section}>
        <div
          ...
          <div
            className={`${styles.sectionHeaderStart} ${openSection === 'preferences' ? styles.active : ''}`}>
      ...
      <section className={styles.section}>
        <div
          ...
          <div
            className={`${styles.sectionHeaderStart} ${openSection === 'productivity' ? styles.active : ''}`}>
            <Zap size={24} />
            <h3>Productivity</h3>
          </div>
      ...
      {showDeleteModal && (
        <div className={styles.overlay}>
          <div className={styles.modal}>
            ...
            <p>
              Type your username ''<strong>{user?.username}</strong>'' to confirm account deletion. This is
