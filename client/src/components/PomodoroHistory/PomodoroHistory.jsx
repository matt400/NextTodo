...
const PomodoroHistory = () => {
	...
	// After panel renders, measure its actual height and clamp to viewport
	useLayoutEffect(() => {
	...
	const handleDelete = async (e, pomoId) => {
		e.stopPropagation();
			...
			{open && createPortal(
				<div
					...
					{!loading && history.length === 0 && (
						<p className={styles.empty}>No pomodoros yet</p>
					)}
								...
								return (
									<li key={p.id} className={styles.entry}>
										...
										<div className={styles.entryMeta}>
											<span className={`${styles.badge} ${completed ? styles.badgeDone : styles.badgeInterrupted}`}>
